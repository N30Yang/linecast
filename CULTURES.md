...
The figures join stars by Hipparcos number; linecast places them from the Hipparcos main catalogue (ESA, CDS I/239) and matches the named stars to its own Yale Bright Star Catalogue by HD number, so a name for a star fainter than magnitude 6.5 is not shown. Figure names are shown in the culture's own language when the app is in that language (Chinese in Chinese, Ruelle's French in French), and otherwise in the English the contributors gave, or the native name where there is no English. Stellarium's Chinese star names are English only, "Northern Pole II", "Curved Array Added IV"; the Chinese name behind each is the asterism's name and an ordinal, 北极二, with 增 for an added star, 勾陈增四, so the bake derives it from the asterism's native name, and takes the few stars named for themselves alone, 太子 for "Crown Prince", and the asterisms the index leaves without a native name from the culture's own zh_CN translation. Chinese Contemporary carries the same star names and is treated as Chinese the same way. The status line names the culture, and the chip under the pointer gives a star's IAU name beside the culture's.
