...
言語は、英語（`en`）、フランス語（`fr`）、スペイン語（`es`）、ポルトガル語（`pt`）、イタリア語（`it`）、ドイツ語（`de`）、オランダ語（`nl`）、デンマーク語（`da`）、ノルウェー語（`no`）、スウェーデン語（`sv`）、アイスランド語（`is`）、フィンランド語（`fi`）、ポーランド語（`pl`）、ウクライナ語（`uk`）、中国語（`zh`）、日本語（`ja`）、韓国語（`ko`）、タイ語（`th`）、ベトナム語（`vi`）、インドネシア語（`id`）です。
